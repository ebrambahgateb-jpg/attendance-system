...
function initNotifications() {
  ...
  if (!currentUser) {
    console.warn('⚠️ Notifications: no user');
    return;
  }

  console.log('🔔 Initializing notifications for:', currentUser.email);
...
function setupNotificationsEvents() {
  ...
  if (bellBtn) {
    ...
    console.log('✅ Notifications bell button bound');
  } else {
    console.warn('⚠️ Notifications bell button not found');
  }
}
    ...
    unsubscribeListener = onSnapshot(q, (snap) => {
      ...
      if (oldIds.length > 0) {
        notificationsData.forEach(n => {
          if (!oldIds.includes(n.id)) {
            if (typeof window.notifyNewNotification === 'function') {
              window.notifyNewNotification(n);
            }
          }
        });
      }
...
window.markAllAsRead = async function() {
  ...
  // ⚡ احفظ في Firestore (بالتوازي)
  const promises = unread.map(n =>
    updateDoc(doc(db, 'notifications', n.id), {
      ReadBy: arrayUnion(currentUser.email)
    }).catch(err => {
      console.warn('⚠️ Mark all error:', err.message);
    })
  );
...
window.deleteNotification = async function(notifId) {
  ...
  // ⚡ حذف فوري من الـState
  const index = notificationsData.findIndex(n => n.id === notifId);
  if (index !== -1) {
...
window.clearAllNotifications = async function() {
  ...
  // ⚡ Toast
  showToast('🗑️ جاري حذف الإشعارات...');

  // ⚡ احذف من Firestore (بالتوازي)
  const deletePromises = notifsToDelete.map(n =>
    deleteDoc(doc(db, 'notifications', n.id)).catch(err => {
      console.warn('⚠️ Clear error for', n.id, ':', err.message);
    })
  );

  await Promise.all(deletePromises);

  showToast(`✅ تم حذف ${notifsToDelete.length} إشعار`);
};
...
function renderEnableNotifButton() {
  ...
  if (notifBtn && notifBtn.parentNode === topbarRight) {
...
function showToast(message) {
  // ⚡ لو في showToast تاني → استخدمه
  if (window._showToastFromChat) {
    window._showToastFromChat(message);
    return;
  }
